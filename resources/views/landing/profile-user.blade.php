        ...
        <div class="container">
            ...
            <div class="row">
                <div class="col-lg-3 col-md-12">
                    <div class="card card_profile text-center">
                            <div class="card-body">
                                <div class="image_profile">
                                    
                                    @if(Auth::user()->profile_picture && Storage::disk('public')->exists('image/user/' . Auth::user()->profile_picture))
                                    <img src="{{asset('storage/image/user/'.Auth::user()->profile_picture)}}" class="rounded-circle border border-2 border-dark" alt="">
                                    @else
                                        <img src="\images\icon\dua.png" class="rounded-circle border border-2 border-dark" alt="">
                                    @endif
                                </div>
                                <h5 class="card-title">{{ Auth::user()->nama_lengkap }}</h5>
                                <button type="button" class="btn btn-dark btn-sm shadow-0" data-mdb-toggle="modal" data-mdb-target="#profileuserModal"><i class="fas fa-edit mr-2"></i>Edit Profile</button>
                            </div>
                    </div>
                </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('profileUser.update', ['id' => Auth::id()]) }}" method="post" enctype="multipart/form-data">
                @csrf
                    <div class="row">
                        <div class="col-md-8">
